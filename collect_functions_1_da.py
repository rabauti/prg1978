...
class DbMethods:
    ...
    key_fields = [
        ...
        {"id": "inf_verb", "type": "text"},
        {"id": "inf_verb_feats", "type": "text"},
        {"id": "inf_verb_deprel", "type": "text"},
        ...
        for key in collocations:
            ...
            sql_colls.append(
                (
                    ...
                    key[4],  # verb_deprel
                    key[5],  # inf_verb
                    key[6],  # inf_verb_feats
                    key[7],  # inf_verb_deprel
                    key[8],  # sentence
                    key[9],  # sentence_id
                )
            )
...
def extract_something(
    ...
    for verb in verb_nodes:
        ...
        # iterate over verb inf verb children
        for da_verb in verb_nodes:
            # kui pole vahetu alluv, siis ei huvita
            if da_verb not in kids:
                continue

            # kui pole da-infinitiiv, siis ei huvita
            if "inf" not in graph.nodes[da_verb]["feats"].keys():
                continue

            inf_verb = graph.nodes[da_verb]["lemma"]
            inf_verb_feats = ",".join(graph.nodes[da_verb]["feats"])
            inf_verb_deprel = graph.nodes[da_verb]["deprel"]

            words = []
            for n in sorted(graph.nodes):
                if not n:
                    continue
                if n in (verb, da_verb):
            ...
            key = (
                ...
                inf_verb,  # inf_verb
                inf_verb_feats,  # inf_verb_feats
                inf_verb_deprel,  # inf_verb_deprel
            ...
            if verb_lemma in ["kibelema", "plaanitsema"]:
                graph.draw_graph2(
                    filename=f"img_da/{collection_id}_{verb_lemma}_{verb_compound}_{inf_verb}.png",
                    highlight=[verb, da_verb],
                )
