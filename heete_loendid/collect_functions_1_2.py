...
def extract_something(graph, collection_id, collocations, verb_global_stat):
    ...
    # ---
    # 3. collect obls
    obj_nodes = graph.get_nodes_by_attributes(attrname="deprel", attrvalue="obj")
        ...
        for nsubj in nsubj_nodes:
            ...
            for xcomp in xcomp_nodes:
                if xcomp not in kids:
                    continue
                if "sup" not in graph.nodes[xcomp]["feats"].keys():
                    continue

                xcomp_lemma = graph.nodes[xcomp]["lemma"]

                has_obj = 0
                for obl in obj_nodes:
                    if obl not in kids:
                        continue
